...
def remove_reachable_rolls(grid_rolls, grid_adjacent_counts)
  rolls_removed = 0

  # Copy the source grids to avoid changing them
  output_grid_rolls = grid_rolls.map { |row| row.dup }
  output_grid_adjacent_counts = grid_adjacent_counts.map { |row| row.dup }

  # Go through each cell and remove reachable rolls
  grid_rolls.each_with_index do |row, row_index|
    row.each_with_index do |cell, column_index|
      next if cell == 0
      next if grid_adjacent_counts[row_index][column_index] >= 4
      rolls_removed += 1
      output_grid_rolls[row_index][column_index] -= 1
      # Decrement all the spaces around the current cell in the adjacent counts grid
      cursor_x = -1
      while cursor_x <= 1 do
        cursor_y = -1
        while cursor_y <= 1 do
          ...
          # Make sure we only edit the cells around the edges if they exist
          next if adjacent_row < 0 || adjacent_col < 0
          next if adjacent_row >= grid_rolls.length || adjacent_col >= grid_rolls[0].length

          # Decrement the count
          output_grid_adjacent_counts[adjacent_row][adjacent_col] -= 1
        end
  ...
  # Recursively call this function until no more rolls can be removed
  if rolls_removed == 0
    return rolls_removed
  end

  rolls_removed += remove_reachable_rolls(output_grid_rolls, output_grid_adjacent_counts)
  return rolls_removed
end
...
removed_rolls_count = remove_reachable_rolls(grid_rolls, grid_adjacent_counts)

puts "\nNumber of rolls removed: #{removed_rolls_count}"